...
// queueNxt -> Scans every record and if match found stores the record
// Input -> Scan, Record
// Output -> RC_CODE
extern RC queueNxt (RM_ScanHandle *scan, Record *record){
	// Perform Global checks 
	// RC is already defined in dberror.h file using it to store RC_CODES
	RC return_code;
...
extern Schema *createSchema (int numAttr, char **attrNames, DataType *dataTypes, int *typeLength, int keySize, int *BTNdKys){	
	Schema *createNewSchema = (Schema *) malloc(sizeof(Schema));
	(*createNewSchema).dataTypes = dataTypes;
	(*createNewSchema).numAttr = numAttr;
	(*createNewSchema).keyAttrs = BTNdKys;
