			...
			if((*fHandle).mgmtInfo != NULL){
				...
				if (((*fHandle).totalNumPages-1) < (*fHandle).curPagePos)
					...
					if(fread(memPage,1,PAGE_SIZE,(*fHandle).mgmtInfo) != PAGE_SIZE)
						...
						int nxtPgNo = ((*fHandle).curPagePos / PAGE_SIZE) + 1;				// Calculate the queueNxt page number by dividing page size by current page position and adding 1.
						Output = readBlock(nxtPgNo, fHandle, memPage);				// Reading the Block by redirecting to ReadBlock() function with queueNxt page number.
						if(Output==0){
