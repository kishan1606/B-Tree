...
testInsertAndFind (void)
{
  ...
  BTNdKys = createValues(stringKeys, numInserts);
  ...
  for(i = 0; i < numInserts; i++)
    TEST_CHECK(insertKey(tree, BTNdKys[i], insert[i]));
...
testDelete (void)
{
  ...
  for(iter = 0; iter < 50; iter++)
	...
	{
	  if (deletes[i])
	    TEST_CHECK(deleteKey(tree, BTNdKys[i]));
	}
...
testIndexScan (void)
{
  ...
  testName = "random insertion BTSOrd and scan";
  ...
  for(iter = 0; iter < 50; iter++)
    {
      ...
      // insert BTNdKys
      for(i = 0; i < numInserts; i++)
	TEST_CHECK(insertKey(tree, BTNdKys[permute[i]], insert[permute[i]]));

      // check index stats
      TEST_CHECK(getNumEntries(tree, &testint));
      ASSERT_EQUALS_INT(numInserts,testint, "number of entries in btree"); // Fixed: Summer 2021 - Old: ASSERT_EQUALS_INT(testint, numInserts, "number of entries in btree"); 
      
      // execute scan, we should see tuples in sort BTSOrd
